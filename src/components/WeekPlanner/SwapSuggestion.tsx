...
export default function SwapSuggestion({ suggestion, meals, onAccept }: SwapSuggestionProps) {
  ...
  return (
    <div className="mt-2 rounded-md bg-amber-50 border border-amber-200 p-2 space-y-1.5">
      <div className="flex items-center gap-1 text-[10px] font-semibold text-amber-700">
        <Lightbulb className="h-3 w-3" />
        Suggested swap
      </div>
      <div className="flex items-center gap-1 text-[10px] text-amber-900">
      ...
      <Button
        size="sm"
        variant="outline"
        className="h-6 text-[10px] px-2 border-amber-300 text-amber-800 hover:bg-amber-100"
        onClick={onAccept}
      >
        Accept swap
      </Button>
