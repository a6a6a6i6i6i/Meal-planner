import type { DayPlan, Meal, MacroTotals, WeekGoals, SwapSuggestion, SlotKey } from "@/types";
...
export function findBestSwap(
  ...
  const usedMealIds = new Set(
    SLOT_KEYS.map((s) => dayPlan[s]?.mealId).filter(Boolean)
  );
  ...
  for (const slot of SLOT_KEYS) {
    ...
    for (const candidate of meals) {
      ...
      const hypothetical: DayPlan = {
        ...dayPlan,
        [slot]: { mealId: candidate.id, servings: entry.servings },
      };
      ...
      if (score < bestScore) {
        bestScore = score;
        const calDelta =
          (candidate.calories - currentMeal.calories) * entry.servings;
        const proteinDelta =
          (candidate.protein - currentMeal.protein) * entry.servings;
        bestSwap = {
          slot,
